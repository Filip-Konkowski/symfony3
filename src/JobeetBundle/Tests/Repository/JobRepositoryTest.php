...
namespace JobeetBundle\Tests\Repository;
...
class JobRepositoryTest extends WebTestCase
{
    ...
    {
        $query = $this->em->createQuery('SELECT c FROM JobeetBundle:Category c');
        $categories = $query->getResult();

        foreach($categories as $category) {
            $query = $this->em->createQuery('SELECT COUNT(j.id) FROM JobeetBundle:Job j WHERE j.category = :category AND j.expiresAt > :date');
            $query->setParameter('category', $category->getId());
            $query->setParameter('date', date('Y-m-d H:i:s', time()));
            $jobs_db = $query->getSingleScalarResult();

            $jobs_rep = $this->em->getRepository('JobeetBundle:Job')->countActiveJobs($category->getId());
    ...
    public function testGetActiveJobs()
    {
        ...
        foreach ($categories as $category) {
            $query = $this->em->createQuery('SELECT COUNT(j.id) from JobeetBundle:Job j WHERE j.expiresAt > :date AND j.category = :category');
            $query->setParameter('date', date('Y-m-d H:i:s', time()));
            $query->setParameter('category', $category->getId());
            $jobs_db = $query->getSingleScalarResult();

            $jobs_rep = $this->em->getRepository('JobeetBundle:Job')->getActiveJobs($category->getId(), null, null);
            ...
            if($jobs_db > 2 ) {
                $jobs_rep = $this->em->getRepository('JobeetBundle:Job')->getActiveJobs($category->getId(), 2);
                ...
                $jobs_rep = $this->em->getRepository('JobeetBundle:Job')->getActiveJobs($category->getId(), 2, 1);
    ...
    {
        $query = $this->em->createQuery('SELECT j FROM JobeetBundle:Job j WHERE j.expiresAt > :date');
        $query->setParameter('date', date('Y-m-d H:i:s', time()));
        $query->setMaxResults(1);
        $job_db = $query->getSingleResult();

        $job_rep = $this->em->getRepository('JobeetBundle:Job')->getActiveJob($job_db->getId());
        ...
        $job_rep = $this->em->getRepository('JobeetBundle:Job')->getActiveJob($job_expired->getId());
