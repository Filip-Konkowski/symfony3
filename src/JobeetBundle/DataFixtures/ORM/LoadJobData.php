...
namespace learnproject\JobeetBundle\DataFixtures\ORM;
...
class LoadJobData extends AbstractFixture implements OrderedFixtureInterface {
    ...
    public function load(ObjectManager $em) {
        ...
        $job_sensio_labs->setExpiresAt(new \DateTime('2016-01-10'));
        ...
        $job_extreme_sensio->setExpiresAt(new \DateTime('2016-01-10'));
        ...
        $job_expired2 = new Job();
        $job_expired2->setCategory($em->merge($this->getReference('category-design')));
        $job_expired2->setType('full-time');
        $job_expired2->setCompany('Sensio Labs');
        $job_expired2->setLogo('sensio-labs.gif');
        $job_expired2->setUrl('http://www.sensiolabs2.com/');
        $job_expired2->setPosition('Web Developer Expired2');
        $job_expired2->setLocation('Warsaw, Poland');
        $job_expired2->setDescription('Lorem ipsum dolor sit amet, consectetur adipisicing elit.');
        $job_expired2->setHowToApply('Send your resume to lorem.ipsum [at] dolor.sit');
        $job_expired2->setIsPublic(true);
        $job_expired2->setIsActivated(true);
        $job_expired2->setToken('job_expired2');
        $job_expired2->setEmail('job2@example.com');
        $job_expired2->setCreatedAt(new \DateTime('2016-01-11'));

        $em->persist($job_sensio_labs);
        $em->persist($job_extreme_sensio);
        $em->persist($job_expired);
        $em->persist($job_expired2);
